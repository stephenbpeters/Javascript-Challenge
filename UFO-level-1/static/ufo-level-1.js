...
function runEnter() {
  ...
  // we only need to filter by date/time for level 1, so turn stuff off here
  ...
  var filteredOtherData3 = data.filter(ufo => ufo.datetime === inputValue3);
...
//   
  filteredOtherData3.forEach(item => {
    var row = tbody.append("tr");
    Object.entries(item).forEach(([key, value]) => {
      var cell = row.append("td");
      cell.text(value);
